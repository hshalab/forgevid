...
describe('runQualityGate', () => {
  ...
  it('passes a clean render with no issues', () => {
    mockRuns(BANNER_OK)
    const report = runQualityGate('/tmp/out.mp4', expected)
  ...
  it('fails closed on zero/unreadable duration', () => {
  ...
  it('flags a resolution mismatch as a warning, not a failure', () => {
  ...
  it('fails when audio was required but no audio stream is present', () => {
  ...
  it('fails on excessive black frames and reports their time ranges', () => {
  ...
  it('fails on excessive frozen frames', () => {
    mockRuns(BANNER_OK, {
  ...
  it('fails when audio is present but mostly silent', () => {
  ...
  it('does not run silencedetect when audio was not required', () => {
    const banner = BANNER_OK.replace('  Stream #0:1: Audio: aac, 44100 Hz, stereo\n', '')
    mockRuns(banner)
    const report = runQualityGate('/tmp/out.mp4', { ...expected, requireAudio: false })
    expect(report.passed).toBe(true)
    expect(report.issues).toEqual([])
  })

  it('skips detection filters the resolved ffmpeg build does not support', () => {
